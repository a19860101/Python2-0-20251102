...
items = htmlfile.find_all('div', class_='l-listTable__tr')
with open('output.txt', 'a', encoding='utf-8')as f:
    for item in items:
        title = item.find('div',class_='c-listTableTd__title')
        print(title)
        if title is not None:
            print(title.find('a').text)
            print(item.find('a',class_='u-username').text)
            print(item.find('div', class_='o-fNotes').text)
            ws.append([
                title.find('a').text,
                item.find('a',class_='u-username').text,
                item.find('div', class_='o-fNotes').text
            ])

wb.save('mobile01.xlsx')
# res.close()
